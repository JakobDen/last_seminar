...
{
public:
    std::complex<double> Julia_const;
    using field_t = std::vector < std::vector < std::size_t > > ;

public:

    explicit Calculator(std::size_t width, std::size_t height, std::size_t max_iterations, std::complex<double> Julia_const) :
		m_width(width), m_height(height), m_max_iterations(max_iterations), Julia_const(Julia_const)
	{
		initialize();
	}
